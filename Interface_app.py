...
data = [
    {"name": "ร้าน A", "location": "ประตู1", "budget": "50 - 100", "type_1": "อาหารตามสั่ง", "time_to_open": "กลางวัน (10โมงเช้า - 4โมงเย็น)"},
    {"name": "ร้าน B", "location": "ประตู1", "budget": "50 - 100", "type_1": "อาหารตามสั่ง", "time_to_open": "กลางวัน (10โมงเช้า - 4โมงเย็น)"},
    {"name": "ร้าน C", "location": "ประตู2", "budget": "100 - 200", "type_1": "ปิ้งย่าง", "time_to_open": "เย็น (ตั้งแต่ 4 โมงเย็น)"},
    {"name": "ร้าน D", "location": "ประตู3", "budget": "200+", "type_1": "อาหารญี่ปุ่น", "time_to_open": "เย็น (ตั้งแต่ 4 โมงเย็น)"}
]
...
# === ส่วนที่ 2: เลือกร้านอาหาร ===
    ...
    if not filtered_df.empty:
        ...
        for index, row in filtered_df.iterrows():
            label = f"{row['name']}"
            detail = f"📌 ประเภท: {row['type_1']}\n📍 บริเวณ: {row['location']}\n💸 งบประมาณ: {row['budget']}\n⏰ เวลาเปิด: {row['time_to_open']}"
            labels.append(f"{label}\n{detail}")
            options.append(row['name'])

        selected = st.radio("✅ เลือกร้านที่ต้องการ", options=options, format_func=lambda x: x)
...
# === ส่วนที่ 3: ขอบคุณและเริ่มใหม่ ===
    ...
    if st.session_state.get('selected_restaurant'):
        st.success(f"🎉 คุณเลือกร้าน: **{st.session_state.selected_restaurant}**")
    else:
