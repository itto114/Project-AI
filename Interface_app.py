...
if st.session_state.step == 1:
    ...
    user_types = pd.unique(pd.concat([df["type_1"]]).dropna())
...
# === ส่วนที่ 2: เลือกร้านอาหาร ===
    ...
    if not filtered_df.empty:
        ...
        for index, row in filtered_df.iterrows():
            ...
            selected = st.checkbox(f"✅ **เลือก {st.markdown(f"### 🏆 **{row['name']}**")}**", key=row['name']
            st.write(f"**ประเภทอาหาร**: {row['type_1']} | **บริเวณ**: {row['location']} | **งบประมาณ**: {row['budget']} | **เวลาเปิดร้าน**: {row['time_to_open']}"))
            if selected:
